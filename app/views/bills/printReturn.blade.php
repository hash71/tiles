...
<div id="page" class="pad15T">
	<div id="bill-head" class="text-center">
        <h2 class="text-center">Product Return Memo</h2>
    @if($salesman_house==2)
            ...
            <div class="form-row pad10T">
				...
				<div class="col-xs-3 text-left">
					<span class="font-bold">Date:</span>
                        {{date("d/m/Y (h:i)",strtotime($singledata->return_date))}}
				</div>
            </div>
	...
	<div id="product-info" class="mrg20T form-bordered">
		<table class="table table-condensed text-center">
            ...
            <tbody>
            <?php $i = 1; ?>
                @foreach($bill_info as $product)
                    <tr>
                        <td>{{$i++}}</td>
                        <td>{{$product->product_id}}</td>
                        <td>{{DB::table('lc_product')->where('product_code',$product->product_id)->pluck('unit_product_size')}}</td>
                        <td class="font-bold sftqty">{{$product->return_quantity}}</td>
                    </tr>
                @endforeach
